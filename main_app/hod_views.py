...
def add_student(request):
    ...
    if request.method == 'POST':
        if student_form.is_valid():
            ...
            start = student_form.cleaned_data.get('start_year')
            end = student_form.cleaned_data.get('end_year')
            # if custom year strings provided, parse and create/get Session object
            if start and end:
                import datetime
                try:
                    s_year = int(str(start).strip().split('-')[0])
                    e_year = int(str(end).strip().split('-')[-1])
                    s_date = datetime.date(s_year, 1, 1)
                    e_date = datetime.date(e_year, 1, 1)
                except Exception:
                    s_date = e_date = datetime.date.today()
                session, _ = Session.objects.get_or_create(start_year=s_date, end_year=e_date)
